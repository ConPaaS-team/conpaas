...
class OpenNebulaNodeDriver(NodeDriver):
	...
	disk_target.text = kwargs['ex_disk_target']
        ...
        hostname = ET.SubElement(context, 'HOSTNAME')
        hostname.text = '$NAME'
        ip_public = ET.SubElement(context, 'IP_PUBLIC')
        ip_public.text = '$NIC[IP]'
        ip_gateway = ET.SubElement(context, 'IP_GATEWAY')
        ip_gateway.text = kwargs['ex_network_gateawy']
        nameserver = ET.SubElement(context, 'NAMESERVER')
        nameserver.text = kwargs['ex_network_nameserver']
