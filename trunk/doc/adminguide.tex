...
three ports should be open for all running instances.  AWS
documentation is available at
...
\item It is particularly important that you use Grub version 2. To
  ...
  \end{code}
\item Edit the first lines of the
...
\item Replace the \verb+occi_templates/common.erb+ OCCI profile from
  your OpenNebula installation with the one from
  \verb+misc/common.erb+. This new version features a number of
  improvements from the standard version:
  \begin{itemize}
...
MySQL database. The easiest way to install them is:

\begin{code}
  apt-get install libapache2-mod-php5 php5-curl php5-mysql \
     mysql-server
\end{code}
