...
class PHPConfigTest(Setup, unittest.TestCase):
  ...
  def test_configCodeVersion(self):
    ...
    self.assertEqual(client.update_php_configuration('localhost', self.server_port, codeVersionId=self.id1),
                     {})
    self.assertEqual(client.get_configuration('localhost', self.server_port),
  ...
  def test_configPHP(self):
    ...
    self.assertEqual(client.update_php_configuration('localhost', self.server_port, phpconf={'upload_max_filesize': '10M'}),
                     {})
    self.assertEqual(client.get_configuration('localhost', self.server_port),
  ...
  def test_configCodeVersionAndPHP(self):
    ...
    self.assertEqual(client.get_configuration('localhost', self.server_port),
                     {'codeVersionId': 'code-default', 'phpconf': self.phpconf_default})
    self.assertEqual(client.update_php_configuration('localhost', self.server_port, codeVersionId=self.id2, phpconf={'upload_max_filesize': '10M'}),
                     {})
    self.assertEqual(client.get_configuration('localhost', self.server_port),
