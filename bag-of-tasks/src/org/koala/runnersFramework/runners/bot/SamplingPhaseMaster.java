    ...
    protected boolean areWeDone() {
        ...
        for (SendPortIdentifier spi : masterRP.connectedTo()) {
            ...
            if ((workers.get(cl).get(node) == null)
                    || /*node did not report job result back yet*/ (workers.get(cl).get(node).isFinished() == false)) {
                return false;
            }
        }
        return true;
...
//                just to see if the vms are shutting down.
//                timer.schedule(new MyTimerTask(this,myIbis,from),60000 * 5);
	...
	private Job findNextJob(Cluster cluster, IbisIdentifier from) {
		...
		/*	if(cluster.isReferenceCluster && 
			...
			if(totalSamplingSubmittedCounter < bot.noSampleJobs-bot.noReplicatedJobs) {
				...
				long timeLeftATU = cluster.timeUnit*60000 - workers.get(clusterName).get(node).getUptime()%(cluster.timeUnit*60000) - 5*60000;
				...
				if(timeLeftATU > 0) {
					...
					/*add timer function*/
					timer.schedule(new MyTimerTask(cluster,myIbis,from),
                                                timeLeftATU);
                                    } catch(Exception e){
		...
		for (Cluster cluster : bot.Clusters.values()) {
			...
			for (WorkerStats ws : wss) {
				...
				System.out.println("Unused fraction of ATU: " + 
						(cluster.timeUnit*60000 - ws.getUptime()%(cluster.timeUnit*60000)));
			}			
		}
...
*/
    ...
    /**
     * Dumps the schedules in BoTRunner.schedulesFile file.
     */
	...
	public void startInitWorkers() {
		...
		for (Cluster c : clusters) {
			System.err.println("BoTRunner has found " + bot.tasks.size()
							+ " jobs; will send " + bot.noReplicatedJobs
							+ " to " + c.pendingNodes
							+ " initial workers on cluster " + c.alias);
			Process p = c.startWorkers(/* deadline2ResTime() */"4:45:00",
