...
public class ExecutionPhaseMaster extends Master {
		...
		protected ExecutionPhaseMaster(BoTRunner aBot, Schedule selectedSchedule) throws Exception {
			super(aBot, aBot.electionName+"-executionPhase");
			electionName=aBot.electionName+"-executionPhase";
					...
					for(WorkerStats ws : workers.get(cluster.alias).values()) {
						if(!ws.isFinished()) {				  
							...
							if(ws.getLatestJobStartTime() == 0) {
								...
								if(!ws.isMarked()) {
									/*check if i should terminateWorker this ATU*/
	...
	*/
						...
						for(Cluster cluster : clusters) {
							...
							if(Mi.intValue() > cluster.prevNecNodes) {
								if(Mi.intValue() > cluster.crtNodes + cluster.pendingNodes) {
									...
									cluster.startWorkers("12:45:00", moreWorkers, electionName, myIbis.properties().getProperty(
		...
		private void selectTerminatedNodes(Cluster cluster) {
			...
			for(int i=orderedByArrival.size()-1; i>=0; i--) {
				...
				if((!ws.isFinished()) && (!ws.isMarked())) {
					...
					if(timeLeftATU < 0) {
						try {						
							InstanceTerminator.terminateWorker(ws.getIbisIdentifier(), cluster.alias, myIbis);
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
					...
					*/
					terminateNode(cluster, ws, " selected by scheduler");
					howManyCrtNodes --;
		...
		private void terminateNode(Cluster cluster, WorkerStats ws, String reason) {
			long crtTime=0;
			...
			if(timeLeftATU <= 0) {
				try {	
                                    cluster.terminateWorker(ws.getIbisIdentifier(), myIbis);
				} catch (IOException e) {
                                    e.printStackTrace();
				}
			} else {
				tt = new MyTimerTask(cluster,myIbis, ws.getIbisIdentifier());
	...
	+ (Math.ceil((double)workers.get(cluster).get(j.getNode()).getUptime() / 60000 / bot.Clusters.get(cluster).timeUnit)  
		...
		for (Cluster c : clusters) {
			Process p = c.startWorkers(/* deadline2ResTime() */"12:45:00",
