#!/bin/bash
SERVER=172.16.93.108
PACKAGE_NAME=conpaas-sql.tar
...
cd src
#PYTHONPATH=$PWD python conpaas/mysql/server/agent/server.py
PYTHONPATH=${DEST_DIR}/src python conpaas/mysql/server/agent/server.py -c ${DEST_DIR}/src/conpaas/mysql/server/agent/configuration.cnf
