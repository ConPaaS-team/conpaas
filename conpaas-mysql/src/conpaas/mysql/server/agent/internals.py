...
class MySQLServerConfiguration:
    ...
    def __init__(self, config, _dummy_config=False):
        ...
        try:
            ...
            self.initiate_mysql(self.conn_username, self.conn_password, self.conn_location)
            ...
            logger.debug('Changing bind-address to mysqld: %s' % config.get("ConPaaSSQL","agent_interface") )
            self.change_config('mysqld', 'bind-address', config.get("ConPaaSSQL","agent_interface")) 
