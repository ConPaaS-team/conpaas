...
\BlankLine
\While{metrics \textbf{in} \emph{data}}{
Compute the weight of metric$_i$, \emph{w$_i$}: \\
\hspace{2mm} \emph{w$_i$} = correlation\_coefficient(metric$_i$, \emph{response\_time})\;
\BlankLine
...
\hspace{3mm} \emph{s\_bck} =\emph{s\_bck} - \emph{w$_i$} \textbf{or} \emph{s\_out} =\emph{s\_out} - \emph{w$_i$}\;\label{alg:thr3}
}}
