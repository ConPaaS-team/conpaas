...
\corina{In the algorithm, shouldn't it be ``if no recent scaling operation'' instead
of ``while''? Also we should probably add the fact that the
algorithm ``sleeps'' for 5 minutes between iterations.}
...
\corina{This algorithm uses several techniques: weighted metrics,
flexible thresholds, trend estimation. I think the name should be
given after the technique that brings the most improvement -- in
this case, trend estimation maybe? Or otherwise choose a more
generic name, not the name of one of the techniques.}
