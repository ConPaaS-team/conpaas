...
In order to benchmark ConPaaS when hosting the Wikipedia services, we
...
\corina{I wouldn't say the traces are heterogeneous in terms of workload mix.
Usually workload mix refers to different types of requests -- e.g. log in,
read article, update article etc. In our case it's most of the time the same
type of request (read an article) but with articles of various complexity.}
