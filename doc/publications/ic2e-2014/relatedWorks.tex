...
In the past few years, more and more cloud providers like Amazon EC2~\cite{amazonEC2} and Windows Azure~\cite{azure-project}, designed and integrated dynamic resource provisioning systems into their infrastructures. Unfortunately, these systems are imprecise and wasteful in terms of SLO fulfillment and resource consumption, as pointed out in~\cite{ghanbari_exploring_2011}. Consequencently, more realistic academic approaches have been proposed. For instance, works~\cite{ali-eldin_2012} and~\cite{bunch_2012} designed provisioning systems that predict the future service demand to decide the amount of resources to be provisioned. Specifically, \cite{bunch_2012} designed a pluggable and cost-aware autoscaling system that supports horizontal and vertical scaling of web applications by analyzing metrics such as the request volume. However, even though these approaches proved to efficiently answer to the question \emph{"When to provision?"}, resource heterogeneity in cloud infrastructures emerged as a new limitation when answering to the question \emph{"How and which type of VM instance to provision?"}.
