...
class OpenNebulaNodeDriver(NodeDriver):
    ...
    def create_node(self, **kwargs):
        ...
        # 'CPU'
        if 'cpu' in kwargs:
            cpu = ET.SubElement(compute, 'CPU')
            cpu.text = kwargs['cpu']

        # 'MEMORY'
        if 'mem' in kwargs:
            memory = ET.SubElement(compute, 'MEMORY')
            memory.text = kwargs['mem']
        ...
        network = ET.SubElement(nic, 'NETWORK', {'href': 'http://%s/network/%s' %(self.connection.host, str(kwargs['network']))})
        
        # 'CONTEXT'
        context = nic = ET.SubElement(compute, 'CONTEXT')
        for elem in kwargs['context']:
            subelem = ET.SubElement(context, elem)
            subelem.text =  kwargs['context'][elem]
