...
class HTCondorAgent(BaseAgent):
    ...
    def __init__(self, config_parser, **kwargs): 
        ...
        **kwargs holds anything that can't be sent in config_parser.
        ...
        # Path to the HTCondor JAR file
        self.htcondor_dir = config_parser.get('agent', 'CONPAAS_HOME')
    ...
    def create_hub(self, kwargs):
        """Create a HTCondor Hub by starting htcondor server with -role hub"""
        ...
        # Starting htcondor hub
        start_args = [ "java", "-jar", "htcondor-server", "-role", "hub" ]

        self.logger.debug("Running command: '%s'. cwd='%s'" % (
            " ".join(start_args), self.htcondor_dir))

        proc = Popen(start_args, cwd=self.htcondor_dir, close_fds=True)

        self.state = 'RUNNING'
        self.logger.info('Hub started up. HTCondor pid=%d' % proc.pid)
        return HttpJsonResponse()

    @expose('POST')
    def create_node(self, kwargs):
        """Create a HTCondor Node. As this host will actually fire up browser
        ...
        # Running the HTCondor Node via xvfb-run and DISPLAY set to :1.  We
	...
	#	to meet your specific service/server needs
        ...
        self.logger.debug("Running command: '%s'. cwd='%s', env='%s'" % (
            " ".join(start_args), self.htcondor_dir, env))

        proc = Popen(start_args, cwd=self.htcondor_dir, env=env, close_fds=True)

        self.state = 'RUNNING'
        self.logger.info('Node started up. HTCondor pid=%d' % proc.pid)
        return HttpJsonResponse()
