...
class Implementation(Base):
	...
	def deploy(self, manager, machines, variables):
		...
		manager.frontend = module_manager._do_startup({'cloud':manager.cloud, 'configuration':instances, 'args': self.__process_args(self.DeploymentArgs, variables), 'code_conf':manager.config})
	...
	def execute(self, manager, machines, variables):
		...
		StartTime = time.time()
		module_manager = manager.module_managers[0]
		success = module_manager._do_run(True, self.__process_args(self.ExecutionArgs, variables))
